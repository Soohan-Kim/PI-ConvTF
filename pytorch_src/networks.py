...
class SAConvLSTM(nn.Module):
    def __init__(self, input_channels, feature_channels, inter_channels, kernel_size, stride, padding, device, last_conv, num_layers):
        ...
        for i in range(num_layers):
            SACellList.append(
                SAConvLSTMCell(input_channels[i], feature_channels[i], inter_channels[i], kernel_size[i], stride[i], padding[i], device)
            )
    ...
    def forward(self, x):
        hidden_sizes = [(x.size(dim=-1) + 2*self.padding[0] - self.kernel_size[0]) / self.stride[0] + 1]
        for i in range(self.num_layers-1):
            hidden_sizes.append((hidden_sizes[i] + 2*self.padding[i+1] - self.kernel_size[i+1]) / self.stride[i+1] + 1)
