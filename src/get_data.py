...
def get_data(m_range, ttm_range, grid, conv_more_inputs, PINN_range=None):
    '''
    ...
    pi_tf_add = np.concatenate((np.expand_dims(temp[:, 0, :, :], axis=-3), temp[:, 2:, :, :]), axis=-3)
    ...
    if conv_more_inputs:
        ...
        cv_more_train, cv_more_valid, cv_more_test = make_dataloader(more_train_x, y_train, more_test_x, y_test, configs.ConvLSTM['batch_size'])
        sa_more_train, sa_more_valid, sa_more_test = make_dataloader(more_train_x, y_train, more_test_x, y_test, configs.SAConvLSTM['batch_size'])
        tf_more_train, tf_more_valid, tf_more_test = make_dataloader(more_train_x, y_train, more_test_x, y_test, configs.ConvTF['batch_size'])
    ...
    # Collect train, test files
    train_files, test_files = collect_files(datafiles)
    test_files = test_files[configs.timestep:]
    ...
    # Sample data points
    train_df = train_df.iloc[PINN_range[0]: PINN_range[1], :].reset_index(drop=True)
